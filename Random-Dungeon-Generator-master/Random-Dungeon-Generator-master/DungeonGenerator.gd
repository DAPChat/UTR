...
func generate(map:TileMap):
	
	var potentialRooms:int = (mapWidth / maxRoomSize) * (mapHeight / maxRoomSize)
	...
	# out of bounds.
	for r in range(-1, mapHeight + 1):
		for c in range (-1, mapWidth + 1):
			map.set_cell(0, Vector2i(c, r), 0, Vector2i(Tiles.SOLID, 0))
	
	# Generate potential rooms.
	for r in potentialRooms:
		position = Vector2(rng.randi_range(1, mapWidth - maxRoomSize), rng.randi_range(1, mapHeight - maxRoomSize))

		key = str(position.x) + " " + str(position.y)
		#key = str(position.x+1) + " " + str(position.y)
		#key = str(position.x) + " " + str(position.y+1)
		#key = str(position.x+1) + " " + str(position.y+1)
		#key = str(position.x+2) + " " + str(position.y)
		#key = str(position.x) + " " + str(position.y+2)
		#key = str(position.x+2) + " " + str(position.y+1)
		#key = str(position.x+1) + " " + str(position.y+2)
		#key = str(position.x+2) + " " + str(position.y+2) #still overlaps bc it only checks original key position

		if (!rooms.has(key)):#need to check for adjacent keys
			room = Room.new()
